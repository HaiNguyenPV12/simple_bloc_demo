...
main() {
  AlbumService albumService = MockAlbumService();
  AlbumBloc albumBloc = AlbumBloc(service: albumService);
  ...
  testWidgets('Should render Appbar with correct title', (tester) async {
    when(albumService.fetchAlbum()).thenAnswer((_) async => mockResponse);
  ...
  testWidgets(
      ...
      (tester) async {
    await tester.pumpWidget(widget);
    albumBloc.emit(AlbumLoadInProgress());
    await tester.pump();
  ...
  testWidgets(
    ...
    await tester.pumpWidget(widget);
    albumBloc.emit(AlbumLoadFailure(errorMessage: errorMessage));
    await tester.pump();
  ...
  testWidgets(
      ...
      (tester) async {
    await tester.pumpWidget(widget);
    albumBloc.emit(AlbumLoadSucess(
  ...
  testWidgets('AlbumCard should be tappable', (tester) async {
    ...
    await tester.tap(albumCardFinder.first);
